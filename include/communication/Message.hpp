...
using MessagePayload =
    std::variant<std::monostate, TelemetryPayload, MovementAuthorityPayload,
                 EmergencyBrakePayload, ReservationPayload, HeartbeatPayload>;
...
struct Message {
  ...
  static Message makeTelemetry(
      std::uint64_t msgId, std::uint32_t senderId, std::uint32_t recipientId,
      SimTimeTick tick, const TelemetryPayload& data,
      MessagePriority priority = MessagePriority::Normal) noexcept;

  [[nodiscard]]
  static Message makeMovementAuthority(
      std::uint64_t msgId, std::uint32_t senderId, std::uint32_t recipientId,
      SimTimeTick tick, const MovementAuthorityPayload& data,
      MessagePriority priority = MessagePriority::High) noexcept;

  [[nodiscard]]
  static Message makeEmergencyBrake(std::uint64_t msgId, std::uint32_t senderId,
                                    std::uint32_t recipientId, SimTimeTick tick,
                                    const EmergencyBrakePayload& data) noexcept;

  [[nodiscard]]
  static Message makeHeartbeat(std::uint64_t msgId, std::uint32_t senderId,
                               SimTimeTick tick) noexcept;
};
