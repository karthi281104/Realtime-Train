...
bool RailwayNetwork::addNode(const Node& node) {
  const auto [iterator, inserted] = nodes_.emplace(node.id(), node);
...
bool RailwayNetwork::addTrack(const Track& track) {
  ...
  if (track.length() <= 0.0 || track.speedLimit() < 0.0) {
    return false;
  }

  tracks_.emplace(track.id(), track);

  adjacency_[track.source()].push_back(track.destination());
...
std::size_t RailwayNetwork::nodeCount() const noexcept { return nodes_.size(); }
...
const std::unordered_map<TrackId, Track>& RailwayNetwork::getAllTracks()
    const noexcept {
  return tracks_;
}

const std::unordered_map<NodeId, Node>& RailwayNetwork::getAllNodes()
    const noexcept {
  return nodes_;
}
...
bool RailwayNetwork::hasCycleFrom(
    NodeId node, std::unordered_map<NodeId, bool>& visited,
    std::unordered_map<NodeId, bool>& recursionStack) const {
  visited[node] = true;
  recursionStack[node] = true;

  const auto adjacencyIterator = adjacency_.find(node);

  if (adjacencyIterator != adjacency_.end()) {
    for (const NodeId neighbour : adjacencyIterator->second) {
      if (!visited[neighbour]) {
        if (hasCycleFrom(neighbour, visited, recursionStack)) {
          return true;
        }
