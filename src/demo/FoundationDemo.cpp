...
const char* nodeTypeToString(infrastructure::NodeType type) {
  switch (type) {
...
void runFoundationDemo() {
  ...
  infrastructure::Node chennai(1, "Chennai Central",
                               infrastructure::NodeType::Station);

  infrastructure::Node arakkonam(2, "Arakkonam",
                                 infrastructure::NodeType::Station);

  infrastructure::Node junction(3, "Railway Junction",
                                infrastructure::NodeType::Junction);

  infrastructure::Node platform(4, "Platform 1",
  ...
  std::cout << "  Node 1: " << chennai.name() << " ("
            << nodeTypeToString(chennai.type()) << ")\n";

  std::cout << "  Node 2: " << arakkonam.name() << " ("
            << nodeTypeToString(arakkonam.type()) << ")\n";

  std::cout << "  Node 3: " << junction.name() << " ("
            << nodeTypeToString(junction.type()) << ")\n";

  std::cout << "  Node 4: " << platform.name() << " ("
  ...
  infrastructure::Track track1(101, 1, 2, 70'000.0, 44.44, 0.5);

  infrastructure::Track track2(102, 2, 3, 40'000.0, 33.33, 0.2);

  infrastructure::Track track3(103, 3, 4, 5'000.0, 22.22, 0.0);
  ...
  std::cout << "  Track 101: Node " << track1.source() << " -> Node "
            << track1.destination() << ", length = " << track1.length()
            << " m, limit = " << track1.speedLimit() << " m/s\n";

  std::cout << "  Track 102: Node " << track2.source() << " -> Node "
            << track2.destination() << ", length = " << track2.length()
            << " m, limit = " << track2.speedLimit() << " m/s\n";

  std::cout << "  Track 103: Node " << track3.source() << " -> Node "
            << track3.destination() << ", length = " << track3.length()
            << " m, limit = " << track3.speedLimit() << " m/s\n";
  ...
  std::cout << "  Tracks      : " << network.trackCount() << '\n';

  std::cout << "  Connected   : "
            << (network.isWeaklyConnected() ? "YES" : "NO") << '\n';

  std::cout << "  Has cycle   : " << (network.hasCycle() ? "YES" : "NO")
            << '\n';
  ...
  if (foundNode != nullptr) {
    std::cout << "  Found node 1: " << foundNode->name() << '\n';
  }

  const auto* foundTrack = network.getTrack(101);

  if (foundTrack != nullptr) {
    std::cout << "  Found track 101: " << foundTrack->source() << " -> "
  ...
  auto express =
      std::make_unique<train::ExpressTrain>(1001, 450'000.0, 55.56, 1.2, 2.5);

  auto passenger =
      std::make_unique<train::PassengerTrain>(1002, 350'000.0, 38.89, 1.0, 2.0);

  auto freight =
      std::make_unique<train::FreightTrain>(1003, 900'000.0, 27.78, 0.8, 1.8);
  ...
  std::cout << "  Train count: " << manager.trainCount() << '\n';

  // ------------------------------------------------------------
  // Display train information
  // ------------------------------------------------------------

  const TrainId trainIds[] = {1001, 1002, 1003};
  ...
  for (const TrainId id : trainIds) {
    ...
    std::cout << "    Type             : " << trainTypeToString(train->type())
              << '\n';

    std::cout << "    Mass             : " << train->mass() << " kg\n";

    std::cout << "    Maximum speed    : " << train->maximumSpeed() << " m/s\n";

    std::cout << "    Service braking  : " << train->serviceBraking()
              << " m/s^2\n";

    std::cout << "    Emergency braking: " << train->emergencyBraking()
              << " m/s^2\n";

    std::cout << "    Position         : " << train->position() << " m\n";

    std::cout << "    Velocity         : " << train->velocity() << " m/s\n";

    std::cout << "    Acceleration     : " << train->acceleration()
              << " m/s^2\n";

    std::cout << "    State            : " << trainStateToString(train->state())
              << '\n';
  }
  ...
  if (expressTrain != nullptr) {
    ...
    std::cout << "  Position     : " << expressTrain->position() << " m\n";

    std::cout << "  Velocity     : " << expressTrain->velocity() << " m/s\n";

    std::cout << "  Acceleration : " << expressTrain->acceleration()
              << " m/s^2\n";
  ...
  std::cout << "Contains train 1001: "
            << (manager.contains(1001) ? "YES" : "NO") << '\n';

  std::cout << "Contains train 9999: "
  ...
  std::cout << "  Remove result: " << (removed ? "SUCCESS" : "FAILED") << '\n';

  std::cout << "  Remaining trains: " << manager.trainCount() << '\n';

  std::cout << "  Contains train 1003: "
  ...
  std::cout << "Module 1 - Infrastructure : OPERATIONAL\n";

  std::cout << "Module 2 - Train Domain   : OPERATIONAL\n";

  std::cout << "Railway nodes             : " << network.nodeCount() << '\n';

  std::cout << "Railway tracks            : " << network.trackCount() << '\n';

  std::cout << "Managed trains             : " << manager.trainCount() << '\n';
