...
Odometer::Odometer(const SensorNoiseConfig& config)
    : config_(config), accumulatedDrift_(0.0), isFaulty_(false) {}

OdometerMeasurement Odometer::measure(
    const DistanceMeters truePosition, const SpeedMetersPerSecond trueVelocity,
    const AccelerationMetersPerSecondSquared trueAcceleration,
    const TimeSeconds dt, const SimTimeTick timestamp) {
  if (isFaulty_) {
  ...
  return OdometerMeasurement{.rawPosition = std::max(0.0, measuredPos),
...
void Odometer::setFaulty(const bool faulty) noexcept { isFaulty_ = faulty; }

bool Odometer::isFaulty() const noexcept { return isFaulty_; }
