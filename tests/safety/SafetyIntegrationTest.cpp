...
prediction::FutureState state(TimeSeconds time, TrackId track)
{
    return {time, track, 0.0, 20.0, 0.0, 1.0};
}

} // namespace

TEST(SafetyIntegrationTest, JunctionConflictProducesPriorityAndFreightResolution)
{
    const auto network = makeNetwork();
    const auto expressTrajectory = std::vector<prediction::FutureState>{
        state(0.0, 101), state(20.0, 102)};
    const auto freightTrajectory = std::vector<prediction::FutureState>{
        state(0.0, 103), state(21.0, 104)};

    const auto conflicts = conflict::ConflictDetector{}.detect(
        1, expressTrajectory, 3, freightTrajectory, network);
    ASSERT_EQ(conflicts.size(), 1U);
    ASSERT_EQ(conflicts.front().type, conflict::ConflictType::Junction);
    ...
    RiskInput riskInput;
    riskInput.timeToCollision = conflicts.front().firstConflictTime;
    riskInput.relativeVelocity = 0.0;
    riskInput.brakingDistance = 250.0;
    riskInput.safetyMargin = -250.0;
    riskInput.conflictType = conflicts.front().type;
    ...
    queue.push(conflicts.front(), RiskEngine{}.assess(riskInput));
    ASSERT_FALSE(queue.empty());

    const auto prioritized = queue.top();
    const PriorityEngine priorityEngine;
    const auto expressPriority = priorityEngine.assess(express);
    const auto freightPriority = priorityEngine.assess(freight);
    ASSERT_TRUE(expressPriority.higherThan(freightPriority));

    const ResolutionInput resolutionInput{
        freight, prioritized.conflict, prioritized.risk, false, true,
        1000.0, 100.0};
    const auto command = ResolutionEngine{}.resolve(resolutionInput);

    EXPECT_EQ(command.trainId, freight.id());
