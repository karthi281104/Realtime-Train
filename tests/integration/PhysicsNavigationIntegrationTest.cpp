...
TEST(PhysicsNavigationIntegrationTest, SafeDistanceOverRouteTrack) {
  ...
  for (const TrackId tid : result.tracks) {
    ...
    const double safe =
        KinematicsEngine::safeDistance(v, braking, track->gradient());
...
TEST(PhysicsNavigationIntegrationTest, UphillRouteSafeDistanceShorterThanFlat) {
  ...
  const double safeFlat = KinematicsEngine::safeDistance(
      v, braking, 0.00, kDefaultReaction, kDefaultMargin);
  const double safeUphill = KinematicsEngine::safeDistance(
      v, braking, 0.02, kDefaultReaction, kDefaultMargin);

  EXPECT_LT(safeUphill, safeFlat);
}

TEST(PhysicsNavigationIntegrationTest,
     DownhillRouteSafeDistanceLongerThanFlat) {
  const SpeedMetersPerSecond v = 25.0;
  const double braking = 0.8;
...
TEST(PhysicsNavigationIntegrationTest,
     RouteTotalDistanceMatchesTrackLengthSum) {
  const RailwayNetwork net = makeTestNetwork();
  RouteNavigator nav;
...
TEST(PhysicsNavigationIntegrationTest,
     FreightTrainBrakingDistanceLongerDueToEmergency) {
...
TEST(PhysicsNavigationIntegrationTest, SafeDistanceExceedsTrackLengthWarning) {
  ...
  const double safeStop =
      KinematicsEngine::safeDistance(30.0, 0.8, track->gradient());
