...
TEST(TrackTest, StoresTrackInformation) {
  Track track(100, 1, 2, 1500.0, 30.0, 0.01);
...
TEST(TrackTest, RejectsZeroLength) {
  EXPECT_THROW(Track(1, 1, 2, 0.0, 30.0, 0.0), std::invalid_argument);
}

TEST(TrackTest, RejectsNegativeLength) {
  EXPECT_THROW(Track(1, 1, 2, -100.0, 30.0, 0.0), std::invalid_argument);
}

TEST(TrackTest, RejectsNegativeSpeedLimit) {
  EXPECT_THROW(Track(1, 1, 2, 1000.0, -10.0, 0.0), std::invalid_argument);
}
...
TEST(TrackTest, AllowsZeroSpeedLimit) {
  EXPECT_NO_THROW(Track(1, 1, 2, 1000.0, 0.0, 0.0));
}
