...
RailwayNetwork createSimpleNetwork() {
  ...
  network.addNode(Node(3, "C", NodeType::Station));
...
TEST(RailwayNetworkTest, AddsNode) {
  ...
  EXPECT_TRUE(network.addNode(Node(1, "A", NodeType::Station)));
...
TEST(RailwayNetworkTest, RejectsDuplicateNode) {
  ...
  EXPECT_FALSE(network.addNode(Node(1, "Duplicate", NodeType::Station)));
...
TEST(RailwayNetworkTest, AddsValidTrack) {
  ...
  EXPECT_TRUE(network.addTrack(Track(100, 1, 2, 1000.0, 30.0, 0.0)));
...
TEST(RailwayNetworkTest, RejectsTrackWithMissingSource) {
  ...
  EXPECT_FALSE(network.addTrack(Track(100, 1, 2, 1000.0, 30.0, 0.0)));
}
...
TEST(RailwayNetworkTest, RejectsDuplicateTrack) {
  ...
  Track track(100, 1, 2, 1000.0, 30.0, 0.0);
...
TEST(RailwayNetworkTest, FindsTrack) {
  ...
  network.addTrack(Track(100, 1, 2, 1000.0, 30.0, 0.0));
...
TEST(RailwayNetworkTest, DFSDetectsCycle) {
  ...
  network.addNode(Node(1, "A", NodeType::Junction));

  network.addNode(Node(2, "B", NodeType::Junction));

  network.addNode(Node(3, "C", NodeType::Junction));

  network.addTrack(Track(101, 1, 2, 1000.0, 30.0, 0.0));

  network.addTrack(Track(102, 2, 3, 1000.0, 30.0, 0.0));

  network.addTrack(Track(103, 3, 1, 1000.0, 30.0, 0.0));
