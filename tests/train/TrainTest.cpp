...
class TestTrain final : public Train {
 public:
  TestTrain(TrainId id, double mass, double maximumSpeed, double serviceBraking,
            double emergencyBraking)
      : Train(id, mass, maximumSpeed, serviceBraking, emergencyBraking) {}

  TrainType type() const noexcept override { return TrainType::Passenger; }
};
...
TEST(TrainTest, RejectsZeroMass) {
  EXPECT_THROW(TestTrain(1, 0.0, 40.0, 0.8, 1.2), std::invalid_argument);
}

TEST(TrainTest, RejectsNegativeMass) {
  EXPECT_THROW(TestTrain(1, -100.0, 40.0, 0.8, 1.2), std::invalid_argument);
}

TEST(TrainTest, RejectsNegativeMaximumSpeed) {
  EXPECT_THROW(TestTrain(1, 50000.0, -1.0, 0.8, 1.2), std::invalid_argument);
}

TEST(TrainTest, RejectsNegativeServiceBraking) {
  EXPECT_THROW(TestTrain(1, 50000.0, 40.0, -0.8, 1.2), std::invalid_argument);
}
...
TEST(TrainTest, RejectsEmergencyBrakingLowerThanServiceBraking) {
  EXPECT_THROW(TestTrain(1, 50000.0, 40.0, 1.2, 0.8), std::invalid_argument);
}
...
TEST(TrainTest, RejectsNegativePosition) {
  ...
  EXPECT_THROW(train.setPosition(-1.0), std::invalid_argument);
}
...
TEST(TrainTest, RejectsVelocityAboveMaximum) {
  ...
  EXPECT_THROW(train.setVelocity(50.0), std::invalid_argument);
}
